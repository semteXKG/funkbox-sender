...
void loop() {
  ...
  bool tx_legal = millis() > last_tx + INTERVAL;
  // Transmit a packet every PAUSE seconds or when the button is pressed
  if (tx_legal) {
    String output = String("ThisIsAMultiByteMesssage" + counter);
    both.printf("TX [%s] ", String(output).c_str());
    radio.clearDio1Action();
    heltec_led(50); // 50% brightness is plenty for this LED
    tx_time = millis();
     RADIOLIB(radio.transmit(output.c_str()));
  ...
  if (millis() > last_timeout_check + 2000) {
    last_timeout_check = millis();
    if (millis() > last_rx + MAX_TIMEOUT) {
      both.println("TIMEOUT REACHED");
    }
  }
